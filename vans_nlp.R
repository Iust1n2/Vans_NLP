...
cname <- file.path("C:/Users/Iustin/Desktop//texts")
cname   
dir(cname)

########
# Change the working directory with the Excel file
setwd("C:/Users/Iustin/Desktop//texts")
...
#
data <- read_excel("Vans - Marketing Strategy.xlsx")
data <-data[,2]
head(data,3)
tail(data)
...
df_1star <- read_excel("Vans - Marketing Strategy.xlsx", sheet = "1 star")
df_2star <- read_excel("Vans - Marketing Strategy.xlsx", sheet = "2 stars")
df_3star <- read_excel("Vans - Marketing Strategy.xlsx", sheet = "3 stars")
df_4star <- read_excel("Vans - Marketing Strategy.xlsx", sheet = "4 stars")
df_5star <- read_excel("Vans - Marketing Strategy.xlsx", sheet = "5 stars")

data_frames <- list(df_1star, df_2star, df_3star, df_4star, df_5star)

###
# The above code block creates multiple dataframes for each separate sheet containing the reviews


# Create a function to preprocess and create DTM
createDtm <- function(data) {
  # Preprocess the text data as needed
  corpus <- Corpus(VectorSource(data$x1)) 
  
  # Text preprocessing steps (customize as needed)
  corpus <- tm_map(corpus, content_transformer(tolower))
  corpus <- tm_map(corpus, removePunctuation)
  corpus <- tm_map(corpus, removeNumbers)
  corpus <- tm_map(corpus, removeWords, stopwords("english"))
  corpus <- tm_map(corpus, removeWords, c("are", "these", "in", "the", "and", "a", "with", "after", "my", "his", "as", "at", "first", "and", "them", "he", "to", "when"))
...
# Apply the createDtm function to each data frame
dtm_1star <- createDtm(df_1star)
dtm_2star <- createDtm(df_2star)
dtm_3star <- createDtm(df_3star)
dtm_4star <- createDtm(df_4star)
dtm_5star <- createDtm(df_5star)
dtms_all <- list(dtm_1star, dtm_2star, dtm_3star, dtm_4star, dtm_5star)


# Assuming you have a list of DTMs for different star categories
dtms_stars <- list(dtm_1star, dtm_2star, dtm_3star, dtm_4star, dtm_5star)

# Define the terms of interest in a vector
terms_of_interest <- c("great", "small", "size")

## Creating the first set of CSV's for the first term of interest
# Loop over different star categories
for (i in 1:length(dtms_stars)) {
  category_name <- paste0(i, "_star") # Create a category name
  cat(paste("Calculating correlations for", category_name, "reviews...\n"))
  
  # Calculate correlations for specific terms in this category
  correlations <- findAssocs(dtms_stars[[i]], "great", corlimit = 0.1)
  
  # Saving the correlations to an Excel file
  write.csv(correlations, paste0(category_name, "_correlations_great.csv"))
}

## Creating another set of CSV's for the second term of interest
...
for (i in 1:length(dtms_stars)) {
  ...
  # Calculate correlations for specific terms in this category
  correlations <- findAssocs(dtms_stars[[i]], "small", corlimit = 0.1)

  # Saving the correlations to an Excel file
  write.csv(correlations, paste0(category_name, "_correlations_small.csv"))
}

## Creating another set of CSV's for the third term of interest
...
for (i in 1:length(dtms_stars)) {
  ...
  # Calculate correlations for specific terms in this category
  correlations <- findAssocs(dtms_stars[[i]], "size", corlimit = 0.1)
  
  # Saving the correlations to an Excel file
  write.csv(correlations, paste0(category_name, "_correlations_size.csv"))
}
...
wordcloud(names(freq), freq, max.words=500, width = 10, height = 5)    
